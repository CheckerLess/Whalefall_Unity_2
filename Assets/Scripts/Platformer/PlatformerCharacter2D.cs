...
namespace UnityStandardAssets._2D
{
    public class PlatformerCharacter2D : MonoBehaviour
    {
        ...
        private bool DoBubbles_Land = false;            // Create bubbles when player lands
        ...
        private ParticleSystem m_LandingParticles;      // Reference to player's landing particle system
        private ParticleSystem m_JumpingParticles;      // Reference to player's Jumping particle system
        private ParticleSystem m_AirJumpingParticles;      // Reference to player's AirJumping particle system
        private ParticleSystem m_MovementParticles;      // Reference to player's Movement particle system
        private Animator m_PlayerSkeleton_Animator;            //PlayerSkeleton
        ...
        private void Awake()
        {
            ...
            m_LandingParticles = transform.Find("Landing_Particles").GetComponent<ParticleSystem>();
            m_JumpingParticles = transform.Find("Jumping_Particles").GetComponent<ParticleSystem>();
            m_AirJumpingParticles = transform.Find("AirJumping_Particles").GetComponent<ParticleSystem>();
            m_MovementParticles = transform.Find("Movement_Particles").GetComponent<ParticleSystem>();
            ...
            m_PlayerSkeleton_Animator = transform.Find("MainCharacter").GetComponent<Animator>();
        ...
        private void FixedUpdate()
        {
            ...
            //if dobubbles is true, set dobubbles to false.
            //if m_grounded returns false, set dobubbles true.
            if (m_Grounded == false)
            {
                DoBubbles_Land = true;
            }

            if ((DoBubbles_Land == true) && (m_Grounded == true))
            {
                DoBubbles_Land = false;
            ...
            m_PlayerSkeleton_Animator.SetBool("Ground", false);
        ...
        public void Move(float move, bool crouch, bool jump, bool sustainJump, bool dash, bool JumpRelease) // Note:this function was updated from the original Move function to add a fifth argument to recognise the DASH. This is called from PlatformerCharacter2DUserControl (Physics based example project).
        {
            ...
            //Release in air.
            //if ((m_Rigidbody2D.velocity.y >= 0f) && (JumpRelease) && (!m_Grounded))
            //{
            ...
            if (m_Grounded || m_AirControl)
            {
                ...
                m_PlayerSkeleton_Animator.SetFloat("Speed", Mathf.Abs(move));
        ...
        public void Death(bool death)
        {
            if (!m_Dead && death)
            {
                ...
                m_PlayerSkeleton_Animator.SetBool("Ground", true);
        ...
        private void PlayMidAirJumpSound()
        {
            ...
            {
                m_MidAirJumpAudioSource.pitch = UnityEngine.Random.Range(0.95f, 1.25f);
